...
void Transmit_To_Bus(struct TX_Payload* data, unsigned char* message){  
  ...
  if(Clear_To_Send()){
    //Serial.println("Clear to send, transmitting message...");
    Print_Message(message, 7 + data->length); // Debug print message
    ...
    Serial.println("Bus busy, cannot send message.");
  }
  delay(50);
}
...
//   Assemble_Message(data, message);
//   digitalWrite(Max485_CE, HIGH);
//   RS485Serial.write(TX_Message, 7 + data->length);  
//   #if !defined(ARDUINO_AVR_NANO)
//   RS485Serial.flush();  //only required for ESP32 
//   #endif
//   digitalWrite(Max485_CE, LOW);
