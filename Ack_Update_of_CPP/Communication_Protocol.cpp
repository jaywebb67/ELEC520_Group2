...
void Assemble_Message(const struct TX_Payload* data, unsigned char* message, const unsigned char* dest, unsigned char* ack) {
  message[0] = START_BYTE;             // Start byte
  message[1] = Home_Address;           // Sender Address byte
  message[2] = *dest;//Destination_Address;    // Destination Address byte
  message[3] = Home_Node_Type;         // type of device e.g. fire, gate etc
  ...
  message[6 + data->length] = *ack;                     // Acknowledgement required?
  message[7 + data->length] = END_BYTE;                 // End byte
}
...
void Transmit_To_Bus(const struct TX_Payload* data, const unsigned char* dest, unsigned char* message){  
  Assemble_Message(data, message, dest);
  digitalWrite(Max485_CE, HIGH);
  RS485Serial.write(TX_Message, 8 + data->length);  
  #if !defined(ARDUINO_AVR_NANO)
...
unsigned char Decode_Message(unsigned char* message, unsigned char* Sender_Address, unsigned char* Sender_Node_Type, unsigned char* payload, unsigned char* ack) {
  ...
  *Sender_Node_Type = message[3];     // Place into variable
  unsigned char length = message[4];  // Use unsigned char for consistency
  *ack = message[6 + length];    // Record ACK/NAK
...
unsigned char Process_RX_Transmission(){
  return Decode_Message(RX_Message, &Sender_Address, &Sender_Node_Type, RX_Message_Payload, &Response); 
}
